...
from architect import Architect_SACN_1, Architect_SACN_2, Architect_SACN_3

parser = argparse.ArgumentParser(description="SACN_1 search model")
...
def main():
    ...
    if args.stage == 1:
        ...
        model = SACN_1(args.init_channels, args.layers, criterion)
    elif args.stage == 2:
        criterion = LossFn_2(face_factor=face_factor,
                             box_factor=bbox_reg_factor, rig_factor=rig_factor)
        model = SACN_2(args.init_channels, args.layers, criterion)
    ...
    if args.stage == 1:
        ...
        train_data = dset.wider(
            anno_file="../anno_store/imglist_anno_24_90.txt", train=True, transform=train_transform)
        valid_data = dset.wider(
            anno_file="../anno_store/imglist_anno_24_90.txt", train=False, transform=valid_transform)
    ...
    if args.stage == 1:
        architect = Architect_SACN_1(model, args)
    elif args.stage == 2:
        architect = Architect_SACN_2(model, args)
...
def train(train_queue, search_queue, model, architect, criterion, optimizer, lr):
    ...
    for step, (input, label, rig, bbox) in enumerate(train_queue):
        ...
        if step % args.report_freq == 0:
            logging.info('train %03d %e %f %f %f', step,
